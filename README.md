...
3. API доступно на `http://localhost:9001` (или `http://45.155.207.118:9001`, если сервис развёрнут на сервере).
...
4. API доступно на `http://localhost:9001` (порт проброшен из контейнера `backend`).
5. Для Android-эмулятора при работе с удалённым сервером используйте `http://45.155.207.118:9001`. При локальной отладке продолжает работать `http://10.0.2.2:9001`, главное пробросить порт `9001:9001`.

## Сборка и запуск Android-клиента
1. Откройте каталог `android/` в Android Studio (Giraffe+).
2. Убедитесь, что устройство/эмулятор могут обратиться к backend (`http://45.155.207.118:9001` — для продакшн-сервера, `http://10.0.2.2:9001` — для локальной отладки; при необходимости измените `ServiceLocator.BASE_URL`).
