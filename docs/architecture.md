...
- **UI.** Отвечает за визуальное представление и взаимодействие с пользователем, отображает состояние и делегирует события в бизнес-логику.
- **Game Logic.** Содержит правила игры, сценарии и расчёты, обеспечивает целостность доменной модели независимо от платформы.
- **Data.** Управляет источниками данных, преобразованием и кэшированием, предоставляет домену унифицированный доступ к хранилищам и сервисам.
- **Integration.** Инкапсулирует инфраструктурные механизмы (DI, коммуникации, вспомогательные сервисы), связывает слои и конфигурирует окружение.
...
- **Генерация ошибок.** Доменные сценарии в пакете `domain.game` возвращают `GameResult<T>` — обёртку над данными, где `Success` содержит актуальный `GameSnapshot`, а `Failure` инкапсулирует типизированный `GameError` (например, `InvalidMove`, `Network`, `DataCorrupted`). Такой подход позволяет Game Logic сохранять контекст ошибки, не смешивая его с механизмами отображения.
- **Адаптация в ViewModel.** `GameViewModel.observeGameState()` подписывается на поток `ObserveGameStateUseCase` и транслирует каждое значение в `LiveData<ViewState>`. Для ошибок применяется явное сопоставление по типам `GameError`, благодаря чему интерфейс получает уже подготовленный `ViewState.Error` с текстами, флагом `isRecoverable` и рекомендуемым действием.
- **Пример отображения.**
  - `GameError.InvalidMove("Клетка занята")` → баннер "Недопустимый ход" с кнопкой «Изменить ход», логирующей событие `retry_invalid_move`.
  - `GameError.Network` → карточка "Проблемы с сетью" с CTA «Повторить» и возможностью повторного запроса.
